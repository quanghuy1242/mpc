 ...
 * 
 * ⚠️ IMPORTANT: WASM Module Isolation
 * ============================================
...
import init, {
  ...
  // ✨ AUTO-GENERATED TYPES from Rust via tsify!
...
function setupLogging() {
  ...
  // ⚠️ IMPORTANT: If using SEPARATE WASM modules (not recommended)
...
async function demonstrateTypeSafeEvents() {
  ...
  // ✨ TypeScript validates these against AUTO-GENERATED types!
  ...
  console.log('\n✓ Type-safe events demonstrated with auto-generated types');
}
...
async function demonstratePassingToRust() {
  ...
  console.log('✓ Raw bus reference obtained:', rawBus);
...
function demonstrateMetadataConfig() {
  ...
  try {
    config.validate();
    console.log('✓ Metadata API config valid');
  } catch (e) {
...
async function demonstrateEventFiltering() {
  ...
  await new Promise(resolve => setTimeout(resolve, 100));
  console.log('✓ Event filtering demonstrated with perfect type safety');
}
...
async function completeExample() {
  ...
  const eventBus = new TypeSafeEventBus(100);
  console.log('✓ Event bus created (type-safe with tsify)');
  ...
  console.log('✓ Features configured:', features.toJson());
  ...
  try {
    ...
    console.log('✓ Metadata API config validated');
  } catch (e) {
    console.warn('⚠ Metadata config validation failed:', e);
  }
    ...
    while (processedEvents < 10) { // Process first 10 events for demo
      try {
        ...
        switch (event.type) {
          case 'Auth':
            if (event.payload.event === 'SignedIn') {
              console.log(`✓ User signed in: ${event.payload.profile_id} via ${event.payload.provider}`);
            } else if (event.payload.event === 'TokenRefreshed') {
              console.log(`🔄 Token refreshed for: ${event.payload.profile_id}`);
            }
            break;
            
          case 'Sync':
            if (event.payload.event === 'Completed') {
              console.log(`✓ Sync completed: +${event.payload.items_added} items, ~${event.payload.items_updated} updated`);
            } else if (event.payload.event === 'Progress') {
              console.log(`⏳ Sync progress: ${event.payload.percent}% (${event.payload.phase})`);
            }
            break;
          ...
          case 'Library':
            if (event.payload.event === 'TrackAdded') {
              ...
              console.log(`♪ Track added: ${event.payload.title} by ${artist}`);
            } else if (event.payload.event === 'PlaylistCreated') {
              console.log(`📝 Playlist created: ${event.payload.name}`);
            }
            break;
          ...
          case 'Playback':
            if (event.payload.event === 'Started') {
              ...
              console.log(`⏸ Paused at ${positionSec}s`);
            }
            break;
        }
      } catch (e) {
